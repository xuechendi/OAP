...
public:
    ...
    if (typed_array->null_count() == 0) {
      for (; cur_id_ < typed_array->length(); cur_id_++) {
        ...
        if (memo_index_to_arrayid_.find(data) == memo_index_to_arrayid_.end()) {
          auto index = memo_index_to_arrayid_size_++;
          memo_index_to_arrayid_[data] = index;
          memo_arrayid_list_.push_back(
              {ArrayItemIndex(cur_array_id_, cur_id_)});
        } else {
          auto index = memo_index_to_arrayid_[data];
          memo_arrayid_list_[index].emplace_back(cur_array_id_, cur_id_);
        }
      }
    } else {
      for (; cur_id_ < typed_array->length(); cur_id_++) {
  ...
  arrow::Status MakeResultIterator(
    ...
    *out = std::make_shared<ProberResultIterator>(
        ctx_, schema, hash_kernel_, &memo_index_to_arrayid_,
        &memo_arrayid_list_, &null_arrayid_,
        //// codegen ////
...
private:
  ...
  dense_hash_map<int64_t, uint64_t> memo_index_to_arrayid_;
  uint64_t memo_index_to_arrayid_size_ = 0;
  std::vector<std::vector<ArrayItemIndex>> memo_arrayid_list_;
  ...
  public:
    ProberResultIterator(
        ...
        dense_hash_map<int64_t, uint64_t> *memo_index_to_arrayid,
        std::vector<std::vector<ArrayItemIndex>> *memo_arrayid_list,
        ...
        : ctx_(ctx), result_schema_(schema), hash_kernel_(hash_kernel),
          ...
          memo_arrayid_list_(memo_arrayid_list), null_arrayid_(null_arrayid) {
      //// codegen ////
    ...
    Process(const ArrayList &in, std::shared_ptr<arrow::RecordBatch> *out,
      ...
      for (int i = 0; i < length; i++) {
        if (!typed_array->IsNull(i)) {
          ...
          // if (index != -1) {
          auto data = typed_array->GetView(i);
          if ((*memo_index_to_arrayid_).find(data) !=
              (*memo_index_to_arrayid_).end()) {
            auto index = (*memo_index_to_arrayid_)[data];
            for (auto tmp : (*memo_arrayid_list_)[index]) {
